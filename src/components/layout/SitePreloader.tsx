    ...
    <AnimatePresence>
      {visible ? (
        <motion.div
          ...
          initial={{ opacity: 1 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0, transition: { duration: reduced ? 0 : 0.42, ease: "easeOut" } }}
          aria-live="polite"
        ...
        >
          ...
          <motion.div
            ...
            initial={reduced ? { scaleX: 1, opacity: 0.85 } : { scaleX: 0, opacity: 0 }}
            animate={{ scaleX: 1, opacity: 0.85, transition: { duration: reduced ? 0 : 0.65, ease: [0.22, 1, 0.36, 1] } }}
            exit={{ scaleX: 0, opacity: 0, transition: { duration: reduced ? 0 : 0.25 } }}
          />
          <motion.div
            className="pointer-events-none absolute left-0 top-[68%] h-px w-full origin-right bg-overlay-text/28"
            initial={reduced ? { scaleX: 1, opacity: 0.85 } : { scaleX: 0, opacity: 0 }}
            animate={{ scaleX: 1, opacity: 0.85, transition: { duration: reduced ? 0 : 0.65, delay: reduced ? 0 : 0.08, ease: [0.22, 1, 0.36, 1] } }}
            exit={{ scaleX: 0, opacity: 0, transition: { duration: reduced ? 0 : 0.25 } }}
          />
          ...
          <motion.div
            ...
            animate={{ opacity: 1, transition: { duration: reduced ? 0 : 0.35 } }}
          ...
          >
            ...
            <div className="relative flex w-full items-center justify-center overflow-hidden py-2">
              <motion.span
                ...
                initial={reduced ? { x: 0, opacity: 1 } : { x: -52, opacity: 0 }}
                animate={{ x: 0, opacity: 1, transition: { duration: reduced ? 0 : 0.7, ease: [0.22, 1, 0.36, 1] } }}
                exit={{ x: reduced ? 0 : -28, opacity: 0, transition: { duration: reduced ? 0 : 0.25 } }}
              >
                Baba
              </motion.span>
              <motion.span
                ...
                animate={{ scaleY: 1, opacity: 1, transition: { duration: reduced ? 0 : 0.5, delay: reduced ? 0 : 0.2 } }}
                exit={{ scaleY: 0, opacity: 0, transition: { duration: reduced ? 0 : 0.2 } }}
              />
              <motion.span
                className="font-display text-5xl leading-none tracking-[-0.04em] text-overlay-text md:text-7xl"
                initial={reduced ? { x: 0, opacity: 1 } : { x: 52, opacity: 0 }}
            ...
            <div className="mt-7 flex w-[min(24rem,80vw)] items-center gap-2 md:mt-8">
              <motion.div
                ...
                animate={{ scaleX: 1, transition: { duration: reduced ? 0 : 0.8, delay: reduced ? 0 : 0.24, ease: [0.22, 1, 0.36, 1] } }}
                exit={{ scaleX: 0, transition: { duration: reduced ? 0 : 0.22 } }}
              />
              <motion.div
                className="h-1.5 w-8 rounded-full bg-accent/90"
                initial={{ opacity: reduced ? 1 : 0 }}
                animate={reduced ? { opacity: 1 } : { opacity: [0.25, 1, 0.25], transition: { duration: 1.2, repeat: Infinity, ease: "easeInOut" } }}
                exit={{ opacity: 0, transition: { duration: reduced ? 0 : 0.2 } }}
              />
              <motion.div
          ...
          <motion.div
            ...
            animate={{ opacity: 0.8, transition: { duration: reduced ? 0 : 0.4 } }}
            exit={{ opacity: 0, transition: { duration: reduced ? 0 : 0.2 } }}
          />
        </motion.div>
      ) : null}
